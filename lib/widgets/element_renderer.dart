...
class ElementRenderer extends StatelessWidget {
  ...
  Widget _buildBadgeImage(String url, {double? height, double? width}) {
    ...
    bool isSvg = badgeUrl.contains('.svg') && !badgeUrl.contains('.png');

    // If it's shields.io, prefer PNG for reliability if not explicitly SVG
    if (badgeUrl.contains('img.shields.io') && !badgeUrl.contains('.svg') && !badgeUrl.contains('/static/v1')) {
       // It likely has .png if generated by our helper, but if user provided raw url without ext, force png
       if (!badgeUrl.contains('.png')) {
          final uri = Uri.parse(badgeUrl);
          // Only append png if not already there or other format
    ...
    if (isSvg) {
      return SvgPicture.network(
        ...
        width: width,
        fit: BoxFit.contain,
        headers: const {'User-Agent': 'ReadmeCreator/1.0'},
        placeholderBuilder: (_) => SizedBox(
          width: width ?? (height != null ? height * 3 : 60),
          height: height ?? 20,
    ...
    return Image.network(
        ...
        errorBuilder: (context, error, stackTrace) {
          ...
          // Try to show something instead of just broken image
          return Container(
             padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
             decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(4)),
             child: const Text('Error', style: TextStyle(fontSize: 10, color: Colors.black)),
          );
        },
      );
  }
