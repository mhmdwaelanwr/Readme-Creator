...
class ElementRenderer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (element is HeadingElement) {
      ...
      double height = 20;
      if (e.badgeStyle == 'for-the-badge') height = 28;

      return _buildBadgeImage(e.imageUrl, height: height);
      ...
      if (e.url.toLowerCase().endsWith('.svg')) {
        ...
        iconWidget = Image.network(
          ...
          loadingBuilder: (context, child, loadingProgress) {
            if (loadingProgress == null) return child;
            return SizedBox(width: e.size, height: e.size, child: const Center(child: CircularProgressIndicator(strokeWidth: 2)));
          },
  ...
  Widget _buildBadgeImage(String url, {double? height, double? width}) {
    // Handle local badge for live preview
    if (url.startsWith('__local_badge__:')) {
      String platformName = url.replaceFirst('__local_badge__:', '');
      return Container(
        ...
        child: Row(
          ...
          children: [
            const Icon(Icons.help_outline, size: 14, color: Colors.red),
            const SizedBox(width: 4),
            Text('Badge غير متوفر: $platformName', style: const TextStyle(fontSize: 10, color: Colors.red)),
          ],
        ),
      );
    }
    // Use a mutable local badgeUrl derived from the parameter
    String badgeUrl = url;
    // Validate URL
    bool isValidUrl = badgeUrl.startsWith('http://') || badgeUrl.startsWith('https://');
    if (badgeUrl.isEmpty || !isValidUrl) {
      return Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(4)),
        child: Row(
          ...
          children: const [
            ...
            SizedBox(width: 4),
            Text('Badge غير متوفر', style: TextStyle(fontSize: 10, color: Colors.red)),
          ],
        ),
      );
    }
    // Check if it is likely an SVG
    // Shields.io (and many badge providers) often return SVG even when the URL
    // doesn't end with .svg. Treat img.shields.io responses as SVG by default
    // unless the URL explicitly references a PNG.
    bool isSvg = false;
    try {
      final uri = Uri.parse(badgeUrl);
      final lower = badgeUrl.toLowerCase();
      if (lower.contains('.svg')) {
        isSvg = true;
      } else if (lower.contains('img.shields.io')) {
        // If the user explicitly requested a png (contains .png) treat as raster,
        // otherwise assume shields returns SVG and use the SVG renderer.
        isSvg = !lower.contains('.png');
      } else if (uri.queryParameters['format'] == 'svg') {
        isSvg = true;
      }
    } catch (_) {
      // If parsing fails, fall back to simple substring checks.
      isSvg = badgeUrl.contains('.svg') && !badgeUrl.contains('.png');
    }

    // If it's an SVG, use SvgPicture for better rendering
    if (isSvg) {
      return SvgPicture.network(
        badgeUrl,
        width: width ?? (height != null ? height * 3 : 50),
        height: height ?? 20,
